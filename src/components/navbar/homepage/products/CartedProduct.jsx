...
const CartedProduct = ({selectedProduct, setSelectedProduct}) => {
    ...
    return (
        <div className='w-full lg:w-[70%] mx-auto border-1 rounded-sm p-10 space-y-3'>
            ...
            {
                selectedProduct.length === 0 ? <NoCartCard></NoCartCard> : selectedProduct.map(product => <CartCard key={product.id} product = {product} selectedProduct = {selectedProduct} setSelectedProduct = {setSelectedProduct} setItems={setItems}></CartCard>)
            }
